(function () {
  ...
  var state = { answers: new Array(QUESTIONS.length).fill(null), i: 0, freeText: '', waitingForText: false };
  ...
  function renderQuestion() {
    ...
    var free = el('process-wrap');
    if (q.freeText && state.waitingForText && state.answers[state.i] === 5) {
      free.innerHTML = '<label for="process">' + esc(q.freeText) + '</label><input id="process" type="text" maxlength="160" value="' + esc(state.freeText) + '"><button type="button" id="free-next">Continue</button>';
      free.className = 'free-answer';
    } else {
      free.innerHTML = '';
      free.className = 'free-answer hidden';
    }
    renderTiles();
  }
  ...
  function answer(value) {
    ...
    if (QUESTIONS[state.i].freeText && value === 5) {
  ...
  el('qcard').addEventListener('click', function (event) {
    ...
    if (event.target.id === 'free-next') {
      state.freeText = (el('process').value || '').trim().slice(0, 160);
      advance();
    }
  });
  el('qback').addEventListener('click', function () {
    if (state.i === 0) return;
    state.i -= 1;
    state.waitingForText = Boolean(QUESTIONS[state.i].freeText && state.answers[state.i] === 5);
    renderQuestion();
  });
  ...
  function analyseOrg() {
    ...
    else if (scoreFor('access') >= 4 && scoreFor('rules') >= 3 && scoreFor('reach') <= 2 && scoreFor('production') <= 2 && scoreFor('spread') <= 2) a.place = 1;
    ...
    if (a.place === 2) {
      ...
      if (rules === 1 || rules === 2) a.placeGap = { spoke: spokeByKey('rules'), score: rules };
        ...
        var order = ['access', 'context', 'ownership', 'spread', 'reach', 'production'];
  ...
  function drawChart(a) {
    ...
    for (var d = 0; d < SPOKES.length; d += 1) {
      ...
      out += '<text class="' + cls + '" x="' + labelX.toFixed(1) + '" y="' + (labelY + 4).toFixed(1) + '" text-anchor="' + anchor + '">' + esc(SPOKES[d].label) + '</text>';
    }
  ...
  function resultCopy(item) {
    ...
    var strong = kind === 'count' ? item.score >= 4 : item.score === 5;
  ...
  function resultActions(code, shareUrl) {
    ...
    if (ORG) {
      ...
      if (state.freeText) html += '<div class="res-block"><span class="label">Process named</span><p id="process-result">' + esc(state.freeText) + '</p></div>';
  ...
  function showResult() {
    ...
    html += '<div><span class="label">Strongest ' + (ORG ? 'condition' : 'way') + '</span>' + (a.strongest ? '<h2 class="strong">' + esc(a.strongest.spoke.label) + '</h2><p>' + esc(ORG ? resultCopy(a.strongest) : a.strongest.spoke.strong) + '</p>' : '<h2 class="strong">None yet</h2><p>Every ' + (ORG ? 'condition is not known' : 'way is new to you') + '.</p>') + '</div>';
