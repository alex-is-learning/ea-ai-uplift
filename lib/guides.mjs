...
function row(item, esc, escAttr, prefix = '') {
  ...
  const mark = item.status === 'guide-planned'
    ...
    : local ? `\n            <a class="guide-read" href="${escAttr(prefix)}guides/${escAttr(item.slug)}/">Read the guide &rarr;</a>` : '';
  const title = local ? `<a href="${escAttr(prefix)}guides/${escAttr(item.slug)}/">${esc(item.title)}</a>` : esc(item.title);
...
function teaser(item, esc, escAttr, prefix) {
  const local = item.status === 'local';
  const href = local ? `${prefix}guides/${escAttr(item.slug)}/` : escAttr(item.links[0].url);
  const rel = local ? '' : ' rel="noopener"';
  const mark = item.status === 'guide-planned' ? ' <span class="guide-mini">planned</span>' : local ? '' : '&nbsp;&#8599;';
...
${rows.map((item) => row(item, esc, escAttr, prefix)).join('\n')}
  ...
  const body = `  <section class="sub-page" aria-labelledby="guides-h1">
    <div class="wrap">
      <a class="back-link" href="${escAttr(prefix)}#guides">&larr; Home</a>
      <p class="legend">Everything in one place</p>
