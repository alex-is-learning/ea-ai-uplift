(() => {
  ...
  const independentGroup = directory.querySelector('[data-independent-group]');
  ...
  function filter() {
    ...
    if (independentGroup) {
      const members = new Set([...independentGroup.querySelectorAll('[data-person-slug]')].filter((row) => !row.hidden).map((row) => row.dataset.personSlug));
      independentGroup.hidden = members.size === 0;
      if (active && members.size) independentGroup.open = true;
      const badge = independentGroup.querySelector('[data-independent-count]');
      badge.textContent = members.size;
      badge.setAttribute('aria-label', quantity(members.size, 'person', 'people'));
    }
  ...
  directory.querySelector('#directory-reset').addEventListener('click', () => {
    ...
    if (independentGroup) independentGroup.open = true;
    search.focus();
  });
  filter();
