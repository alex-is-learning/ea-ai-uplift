...
.ask-rows{margin:22px 0 0; padding:0; list-style:none; max-width:74ch}
.ask-row{display:flex; flex-wrap:wrap; align-items:baseline; gap:4px 14px; padding:10px 0; border-top:1.5px dotted var(--tang)}
.ask-rows li:last-child{border-bottom:1.5px dotted var(--tang)}
.ask-row-title{font-family:var(--display); font-weight:700; font-size:16px; color:var(--ink); text-decoration:none;
  box-shadow:inset 0 -2px 0 0 rgba(18,51,204,.35)}
.ask-row-title:hover{color:var(--tang-ink); box-shadow:inset 0 -2px 0 0 var(--tang)}
.ask-row-meta{font-size:13.5px; color:var(--ink-2)}
.ask-post{margin-top:36px; padding-top:24px; border-top:1.5px dotted var(--tang); max-width:60ch}
.ask-post .cta{margin-top:0}
.ask-post .ask-how{margin-top:18px; font-size:16.5px; color:var(--ink-2)}
.help-options{display:grid; grid-template-columns:repeat(3,minmax(0,1fr)); gap:16px; margin:28px 0 38px; padding:0; list-style:none}
.help-option{padding:20px; border:1.5px solid var(--ink); background:var(--paper-2)}
.help-option h2{font-size:20px}
.help-option p{margin-top:10px; color:var(--ink-2)}
.help-option .more{display:inline-block; margin-top:16px; font-family:var(--display); font-weight:700}
.ask-public{padding-top:34px; border-top:1.5px dotted var(--tang)}
.ask-public .lede{margin-top:10px}
@media (max-width:390px){
  .ask-card h3{font-size:18px}
  .ask-card .ask-meta{letter-spacing:.06em}
  .help-options{grid-template-columns:minmax(0,1fr); gap:12px; margin-bottom:30px}
  .help-option{padding:18px}
}
`;
...
function teaser(item, esc, escAttr) {
  return `        <li class="ask-row">
          <a class="ask-row-title" href="${escAttr(item.contact)}" rel="noopener">${esc(item.title)}</a>
          <span class="ask-row-meta">${esc(item.postedBy)} &middot; ${BUDGET_WORD[item.budget]}</span>
        </li>`;
}

// The home page carries a short list; the board with context, dates and the
// posting notes lives at asks/.
export function section({ items, site, esc, escAttr }) {
  const count = items.length;
  const intro = count === 0
    ? 'No public requests are live yet. You can contact a practitioner directly or submit a reviewed public request.'
    : `${countWord(count, true)} open ${count === 1 ? 'ask' : 'asks'}. Each is a problem, not a brief. Anyone listed above can reply.`;
  const list = count === 0 ? '' : `
      <ul class="ask-rows">
${items.map((item) => teaser(item, esc, escAttr)).join('\n')}
      </ul>`;
  ...
  <section class="ask-board" id="asks" aria-labelledby="asks-title">
    <div class="wrap">
      ...
      <p class="sec-links"><a class="more" href="asks/">Help options &rarr;</a><a class="more" href="${escAttr(site.askFormUrl)}">Post a public ask &rarr;</a></p>
    </div>
  </section>`;
}
...
export function pages(ctx) {
  ...
  const count = items.length;
  const board = count === 0
    ? `      <p class="cards-note">No requests are live yet.</p>`
...
${items.map((item) => card(item, esc, escAttr)).join('\n')}
  ...
  const body = `  <section class="sub-page ask-board" aria-labelledby="asks-h1">
    <div class="wrap">
      <a class="back-link" href="${escAttr(prefix)}">&larr; Home</a>
      <p class="legend">Three routes</p>
      <h1 id="asks-h1">Get help with a real problem</h1>
      <p class="lede">Choose direct contact when you do not want to publish the problem on this board. Use a public request only when its details and contact link can appear online.</p>
      <ul class="help-options">
        <li class="help-option">
          <h2>Contact a practitioner directly</h2>
          <p>Open a profile and use its contact link. This action does not create a public Help wanted request.</p>
          <a class="more" href="${escAttr(prefix)}people/">Find a practitioner &rarr;</a>
        </li>
        <li class="help-option">
          <h2>Assess your organisation</h2>
          <p>Answer ten questions to identify one condition that can help your organisation use AI more effectively.</p>
          <a class="more" href="${escAttr(prefix)}assess/org/">Start the organisation assessment &rarr;</a>
        </li>
        <li class="help-option">
          <h2>Publish a request</h2>
          <p>Submit a problem for maintainer review. If accepted, the request and its contact link appear on this public board.</p>
          <a class="more" href="${escAttr(site.askFormUrl)}" rel="noopener">Post a public request &rarr;</a>
        </li>
      </ul>
      <div class="ask-public" aria-labelledby="public-requests-title">
        <p class="legend">The asks</p>
        <h2 id="public-requests-title">Public requests</h2>
        <p class="lede">A maintainer reviews each request before publication. Practitioners reply through the public contact URL that the poster provides. Publication does not guarantee a reply.</p>
${board}
      </div>
      <div class="ask-post">
        <p><a class="cta" href="${escAttr(site.askFormUrl)}" rel="noopener">Post a public request &rarr;</a></p>
        <p class="cta-host">${esc(formHost)}</p>
        <p class="ask-how">Write the pain, not the tool. Give one sentence, optional context, and a public contact URL. The form requires a budget label. Choose &ldquo;unstated&rdquo; if you do not want to state one. A maintainer asks for approval of the final public text before publication.</p>
      </div>
    </div>
  </section>`;
  return [{
    path: 'asks/index.html',
    html: render({ title: 'Get help — EA AI Uplift', description: 'Contact a practitioner directly, assess an organisation, or submit a reviewed public request for help.', canonical: 'https://eaaiuplift.com/asks/', prefix, nav: ctx.nav ?? '', css: ctx.sectionCss ?? '', body }),
  }];
}
