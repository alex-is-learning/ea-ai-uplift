...
${items.map((item) => card(item, esc, escAttr)).join('\n')}
  ...
  const body = `  <section class="sub-page ask-board" aria-labelledby="asks-h1">
    <div class="wrap">
      ...
      <p class="lede">${count === 0 ? 'When an organisation or an individual in this community has something slow or painful, it appears here.' : `${countWord(count, true)} open ${count === 1 ? 'ask' : 'asks'} from organisations and individuals in this community. Each one is a problem, not a brief.`} Anyone listed in the <a href="${escAttr(prefix)}people/">people directory</a> can reply. The question to answer is &ldquo;what is slow or painful right now?&rdquo;, not &ldquo;what should be built?&rdquo;.</p>
${board}
      <div class="ask-post">
