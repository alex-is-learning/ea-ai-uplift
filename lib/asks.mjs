...
export function section({ items, site, esc, escAttr }) {
  ...
  const intro = count === 0
    ? 'Nothing is posted right now. Problems from organisations and individuals in this community appear here, and anyone listed above can reply.'
...
${items.map((item) => teaser(item, esc, escAttr)).join('\n')}
  ...
  <section class="ask-board" id="asks" aria-labelledby="asks-title">
    <div class="wrap">
      ...
      <p class="sec-links"><a class="more" href="asks/">All asks &rarr;</a><a class="more" href="${escAttr(site.askFormUrl)}">Post an ask &rarr;</a></p>
    </div>
  </section>`;
}
...
export function pages(ctx) {
  ...
  const count = items.length;
  const board = count === 0
    ? `      <p class="cards-note">Nothing is posted right now.</p>`
...
${items.map((item) => card(item, esc, escAttr)).join('\n')}
  ...
  const body = `  <section class="sub-page ask-board" aria-labelledby="asks-h1">
    <div class="wrap">
      <a class="back-link" href="${escAttr(prefix)}#asks">&larr; Home</a>
      <p class="legend">The asks</p>
      <h1 id="asks-h1">Help wanted</h1>
      <p class="lede">${count === 0 ? 'When an organisation or an individual in this community has something slow or painful, it appears here.' : `${countWord(count, true)} open ${count === 1 ? 'ask' : 'asks'} from organisations and individuals in this community. Each one is a problem, not a brief.`} Anyone listed on the <a href="${escAttr(prefix)}#people">home page</a> can reply. The question to answer is &ldquo;what is slow or painful right now?&rdquo;, not &ldquo;what should be built?&rdquo;.</p>
${board}
      <div class="ask-post">
        <p><a class="cta" href="${escAttr(site.askFormUrl)}">Post an ask &rarr;</a></p>
        <p class="cta-host">${esc(formHost)}</p>
        <p class="ask-how">Write the pain, not the tool: one sentence on what is slow, repeated or dropped, a little context, and a public link where someone can reach you. Budget is optional and only ever a word, never a number.</p>
      </div>
    </div>
  </section>`;
  return [{
    path: 'asks/index.html',
    html: render({ title: 'Help wanted — EA AI Uplift', description: 'Open asks from organisations and individuals in the effective-altruism community. Each one is a problem, not a brief.', canonical: 'https://eaaiuplift.com/asks/', prefix, nav: ctx.nav ?? '', css: ctx.sectionCss ?? '', body }),
  }];
}
