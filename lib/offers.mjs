...
export const FIELDS = ['slug', 'name', 'by', 'providerType', 'kind', 'summary', 'detail', 'url', 'access', 'audience', 'listed', 'checked'];
...
function courseRow(o, esc, escAttr) {
  return `        <li class="offer-course" data-offer="${escAttr(o.slug)}">
          ...
          <p class="offer-course-meta">${esc(word(o.access))} &middot; ${esc(AUDIENCE_WORD[o.audience])} &middot; Checked ${esc(longDate(o.checked))}</p>
        </li>`;
}
  ...
  <section id="offers" aria-labelledby="offers-title">
    <div class="wrap">
      <div class="sec-head">
        ...
        <p class="intro">Courses, tools and programmes that help this community use AI well. Hand-picked, not endorsed.</p>
      </div>${list}
...
export function pages(ctx) {
  ...
  const personal = items.filter((item) => item.kind !== 'course' && item.providerType === 'person');
  const organisations = items.filter((item) => item.kind !== 'course' && item.providerType === 'organisation');
...
${cardGroup({ className: 'offer-group-organisations', title: 'Organisation offers', description: 'Made and run by organisations.', items: organisations, esc, escAttr })}
...
`;
  const body = `  <section class="sub-page" aria-labelledby="offers-h1">
    <div class="wrap">
      ...
      <p class="lede">Courses, tools, cohorts, products, programmes and communities that help people and organisations in this community use AI well. Hand-picked because they are useful here. A listing is not an endorsement. Anyone can propose one.</p>
${grid}      <p><a class="cta" href="${escAttr(site.offerFormUrl)}">List an offer &rarr;</a></p>
  ...
  return [{
    ...
    html: render({ title: 'Offers — EA AI Uplift', description: 'Courses, tools, cohorts, products, programmes and communities that help people and organisations in the effective-altruism community use AI well.', canonical: 'https://eaaiuplift.com/offers/', prefix, nav: ctx.nav ?? '', css: ctx.sectionCss ?? '', body }),
  }];
}
