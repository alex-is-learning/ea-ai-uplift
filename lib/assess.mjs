...
import { affiliationTags } from './shared.mjs';
...
// The three connective questions are yes-or-no facts, not degrees of habit.
const YES_NO = [
  [5, 'Yes'],
  [3, 'Not sure'],
  [1, 'No'],
...
const PLACES = [
  { n: 1, title: 'Start from the chat window', because: 'Chat is established while the other methods score lower. Complete one checked task before you add more tools.' },
  { n: 2, title: 'Build one repeatable practice', because: 'Your known answers show a next area to practise. Start with one bounded task.' },
  { n: 3, title: 'Explore unfamiliar methods', because: 'Three or more methods are unknown. This does not show that they are weak or that they solve a current problem.' },
  { n: 4, title: 'Ask for help with the stuck task', because: 'You named a task that you want to hand to AI but have not started.' },
  { n: 5, title: 'Make the practice reusable', because: 'Several advanced methods score highly. The next step is evidence that another person can use the work.' },
];
...
.assess-row{display:grid; grid-template-columns:minmax(0,1fr) auto; gap:var(--gut) 40px; align-items:end}
.assess-go{margin:0}
.assess-band .cta{margin-top:0}
...
.question-context{margin-top:14px}
.question-context label{display:block; margin-bottom:6px; font-family:var(--display); font-size:13px; font-weight:700; color:var(--ink-2)}
.question-context input{width:100%; padding:9px 12px; border:1.5px solid var(--ink); background:var(--paper); color:var(--ink); font:16px/1.35 var(--body)}
...
.result-primary{margin-top:10px; padding:20px; border:2px solid var(--cobalt); background:rgba(18,51,204,.06)}
.result-primary h2{margin-top:7px; font-size:clamp(24px,2.6vw,34px); color:var(--cobalt-deep)}
.result-primary p{margin-top:8px; max-width:52ch; font-size:16px; line-height:1.5}
.result-primary .cta{margin-top:16px}
.result-analysis{margin-top:26px; border-top:1.5px dotted var(--tang); border-bottom:1.5px dotted var(--tang); padding:16px 0}
.result-analysis summary{font-family:var(--display); font-weight:800; font-size:17px; color:var(--cobalt); cursor:pointer}
.result-analysis-body{padding-bottom:8px}
...
.save-notes{margin:14px 0 10px; font-size:15px}
.save-notes label{display:flex; gap:8px; align-items:flex-start}
.save-notes input{margin-top:3px}
.save-result{border:0; cursor:pointer; font-family:var(--display)}
.result-meta{margin-top:18px; font-family:var(--display); font-size:13px; font-weight:700; color:var(--ink-2)}
...
.assess-switch{margin-top:12px; font-family:var(--display); font-size:14.5px; font-weight:700}
.team-code{display:grid; grid-template-columns:minmax(0,1fr) auto; gap:8px; margin-top:10px}
.team-code input{width:100%; min-width:0; padding:9px 12px; border:1.5px solid var(--ink); background:var(--paper-2); color:var(--ink); font:15px/1.4 var(--body)}
.team-code button{font-family:var(--display); font-weight:800; border:0; background:var(--cobalt); color:#fff; padding:9px 14px; cursor:pointer}
...
@media (min-width:1001px){
  ...
  body.assessment-active .site-foot{display:none}
  body.assessment-active .assess-page{height:calc(100vh - 55px); padding:14px 0; overflow:hidden}
  body.assessment-active .assess-page>.wrap{height:100%; display:flex; flex-direction:column}
  body.assessment-active .back-link{margin-bottom:8px}
  body.assessment-active .assess-grid{flex:1; min-height:0; grid-template-columns:minmax(0,.8fr) minmax(0,1.2fr); gap:var(--gut)}
  body.assessment-active .assess-grid>div:first-child .assess-lede,
  body.assessment-active .assess-grid>div:first-child .assess-fine{display:none}
  body.assessment-active .assess-page h1{font-size:clamp(30px,3vw,38px); max-width:18ch}
  body.assessment-active .spokes{margin-top:14px; padding:12px}
  body.assessment-active .spoke-tiles li{min-height:clamp(120px,28vh,185px)}
  body.assessment-active .qcard{top:68px; padding:16px 20px 18px}
  body.assessment-active .qhead{padding-bottom:8px}
  body.assessment-active .qspoke{margin-top:10px}
  body.assessment-active .qstatement{margin-top:6px; font-size:clamp(19px,1.7vw,23px); line-height:1.18}
  body.assessment-active .question-context{margin-top:10px}
  body.assessment-active .question-context input{padding:7px 10px; font-size:16px; line-height:1.25}
  body.assessment-active .answers{margin-top:12px; gap:5px}
  body.assessment-active .answers button{padding:7px 12px; font-size:16px; line-height:1.2}
  body.assessment-active .answers li.dk{margin-top:2px}
  body.assessment-active .qfoot{margin-top:7px; font-size:13px}
  body.assessment-active .qmove{display:none}
}
@media (max-width:1000px){
  .assess-grid{grid-template-columns:minmax(0,1fr)}
  .assess-interaction{order:1}
  .assess-overview{order:2}
    ...
    <div class="wrap">
      <div class="assess-row">
        <div class="sec-head">
          ...
          <p class="intro">Rate ten plain statements and see which of seven ways of working are strong, thin, or not met yet. No account, nothing stored.</p>
        </div>
        <p class="assess-go"><a class="cta" href="assess/">Map your practice &rarr;</a></p>
        <p class="assess-go"><a href="assess/org/">For your organisation &rarr;</a></p>
      </div>
    </div>
  </section>`;
}
...
function personCard(p, esc, escAttr, prefix) {
  ...
  return `          <li class="p-card">
            ...
            <p class="p-tags" aria-label="Work affiliation">${affiliationTags(p).map((tag) => `<span>${esc(tag)}</span>`).join('')}</p>
            ${p.publicationBasis === 'public-sources-pending-review' ? '<p class="draft-note">Public-source draft</p>' : ''}
            <p class="p-head">${esc(p.headline)}</p>
          </li>`;
}

export function assessmentPages(ctx, options = {}) {
  const { items, people, esc, escAttr } = ctx;
  const render = ctx.renderPage ?? (() => '');
  const org = options.kind === 'org';
  ...
  const connectives = options.connectives ? Object.fromEntries(Object.entries(options.connectives).map(([tag, item]) => {
    if (!item.guide) return [tag, item];
    const guide = byGuide[item.guide];
    if (!guide) throw new Error(`assess-org connective ${tag}: guide ${item.guide} is not an entry in data/guides`);
    return [tag, {
      copy: item.copy,
      route: { title: item.routeTitle, href: guideTarget(guide, prefix), local: guide.status === 'local' },
    }];
  })) : null;
  const answerKinds = options.answerKinds ?? { scale: SCALE, yesno: YES_NO };
  const data = {
    kind: options.kind ?? 'individual',
    version: '0.2',
    spokes: spokes.map((s) => ({
      key: s.key, label: s.label, short: s.short, strong: s.strong, middle: s.middle ?? null, edge: s.edge, unmet: s.unmet,
      route: s.guide
        ? { title: options.routeTitles?.[s.key] ?? byGuide[s.guide].title, why: byGuide[s.guide].why, href: guideTarget(byGuide[s.guide], prefix), local: byGuide[s.guide].status === 'local' }
        : { ...s.route, local: !/^https:\/\//u.test(s.route.href) },
    })),
    questions: questions.map((q) => ({
      n: q.order, spoke: q.spoke, tag: q.tag, statement: q.statement,
      answers: Array.isArray(q.answers) ? q.answers : answerKinds[q.answers ?? (q.spoke ? 'scale' : 'yesno')],
      answerKind: Array.isArray(q.answers) ? (q.spoke === 'rules' ? 'document' : 'staged') : (q.answers ?? (q.spoke ? 'scale' : 'yesno')),
      notSureFirst: q.notSureFirst ?? false,
    ...
    connectives,
    routes: {
      beginner: `${prefix}guides/first-useful-task/`,
      progression: `${prefix}guides/practitioner-pathway/`,
  ...
  const sortedPeople = (people || []).slice().sort((a, b) => {
    if (org && (a.availability === 'available') !== (b.availability === 'available')) return a.availability === 'available' ? -1 : 1;
    return a.name < b.name ? -1 : 1;
  });
  const body = `  <section class="assess-page" aria-labelledby="assess-h1">
    <div class="wrap">
      <a class="back-link" href="${options.backHref ?? prefix}">&larr; Back to the guide</a>
      <div class="assess-grid">
        <div class="assess-overview">
          <p class="legend">${org ? 'Your organisation' : 'Your AI practice'}</p>
          <h1 id="assess-h1">${org ? 'Which conditions are strong, and which are not known?' : 'Which ways are strong, and which have you not met?'}</h1>
          <p class="assess-lede">${org ? 'Ten questions for an operations lead, COO, chief of staff or executive director. Each answer is a fact about what the organisation provides and knows.' : 'Seven statements about how you work with these tools, then three short questions. Rate each statement as it is today, not as you would like it to be. If a statement describes something you have never met, say so: that is the most useful answer on the page.'}</p>
          <p class="assess-fine">${org ? '<b>Staff</b> means paid staff and long-term contractors, including the respondent, not volunteers or trustees. <b>Out of date</b> means not touched in the last twelve months. <b>Regular or power users</b> use AI for real work each week; occasional users use it less often.' : 'No account. Nothing leaves this page unless you choose to send yourself the result. The seven ways are a working model from one practitioner, version 0.1, and will change as evidence comes in.'}</p>
          ${org ? '' : '<p class="assess-switch"><a href="org/">Assess an organisation instead &rarr;</a></p>'}
...
${spokes.map((s) => `              <li data-key="${escAttr(s.key)}"><i></i><span>${esc(s.label)}</span></li>`).join('\n')}
          ...
          <div class="chart-wrap hidden" id="chart-wrap">
            <svg class="chart" id="chart" viewBox="0 0 520 480" role="img" aria-labelledby="chart-title"><title id="chart-title">${org ? 'Seven organisation conditions, drawn as a chart' : 'Your seven ways, drawn as a chart'}</title></svg>
            <p class="chart-key" id="chart-key"></p>
          </div>
        </div>
        <div class="assess-interaction">
          <div class="qcard" id="qcard">
            ...
            <div class="question-context">
              <label id="context-label" for="context-input">Optional context</label>
              <input id="context-input" type="text" maxlength="240" autocomplete="off" placeholder="Add a note if useful">
            </div>
            <ul class="answers" id="answers"></ul>
            <p class="qfoot hidden" id="tie-rule"></p>
            <p class="qfoot hidden" id="question-note"></p>
            <p class="qfoot qmove">One tap moves you on. Back takes you to the last question.</p>
          </div>
          <div class="result hidden" id="result"></div>
          ${org ? '' : '<p class="hidden" id="org-entry"><a href="org/">Do you run an organisation?</a></p>'}
        </div>
      </div>
      <div class="res-people hidden" id="res-people">
        <div class="sec-head">
          ...
          <p class="intro">${org ? 'Three ways to get this: a practitioner, an in-house role, protected time.' : 'The directory includes in-house and independent people. Open a profile for work context, availability and contact. Listed alphabetically. Nobody is first.'}</p>
        </div>
...
${sortedPeople.map((p) => personCard(p, esc, escAttr, prefix)).join('\n')}
  ...
  return [{
    path: options.path ?? 'assess/index.html',
    html: render({
      title: options.title ?? 'Your AI practice — a ten-question self-assessment — EA AI Uplift',
      description: options.description ?? 'Ten plain statements, seven ways of working with AI tools, one chart. See which ways are strong, which are thin, and which you have not met yet.',
      canonical: options.canonical ?? 'https://eaaiuplift.com/assess/',
      prefix,
      nav: ctx.nav ?? '',
      css: ctx.sectionCss ?? '',
      body,
