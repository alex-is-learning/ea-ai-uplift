...
.free-answer{margin-top:14px; padding:14px; border:1.5px dotted var(--tang); background:var(--paper-2)}
.free-answer label{display:block; font-family:var(--display); font-weight:700; margin-bottom:8px}
.free-answer input{width:100%; padding:10px 12px; border:1.5px solid var(--ink); background:var(--paper); color:var(--ink); font:16px/1.4 var(--body)}
.free-answer button{margin-top:10px; font-family:var(--display); font-weight:800; border:0; background:var(--cobalt); color:#fff; padding:10px 16px; cursor:pointer}
...
${spokes.map((s) => `              <li data-key="${escAttr(s.key)}"><i></i><span>${esc(s.label)}</span></li>`).join('\n')}
        ...
        <div>
          <div class="qcard" id="qcard">
            ...
            <div class="free-answer hidden" id="process-wrap"></div>
