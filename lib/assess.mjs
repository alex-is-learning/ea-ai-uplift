...
${spokes.map((s) => `              <li data-key="${escAttr(s.key)}"><i></i><span>${esc(s.label)}</span></li>`).join('\n')}
      ...
      <div class="res-people hidden" id="res-people">
        <div class="sec-head">
          ...
          <p class="intro">${org ? 'Three ways to get this: a practitioner, an in-house role, protected time.' : 'Each person works independently and sets their own terms. Open a profile for availability and how to get in touch. Listed alphabetically. Nobody is first.'}</p>
        </div>
