...
const FOOTER_FINE =
  'Maintained by Alexander Large. The directory includes in-house and independent work, but the people listed do not form a team. Nothing here is affiliated with, or endorsed by, any effective-altruism organisation or employer. &ldquo;EA&rdquo; names the community this guide is written for. This page sets no cookies, runs no analytics and collects no personal data.';

function footer(prefix = '') {
  return `<footer class="site-foot">
  <div class="wrap">
    <p class="mark-f">EA AI Uplift</p>
    <nav class="foot-nav" aria-label="More ways to use this guide">
      <a href="${escAttr(prefix)}start/">Start here</a>
      <a href="${escAttr(prefix)}learn/">Learn to do this work</a>
...
.home-hub{padding:46px 0 52px}
.home-grid{display:grid; grid-template-columns:minmax(240px,.72fr) minmax(460px,1.28fr); gap:46px 72px; align-items:start}
.home-copy h1{max-width:14ch}
.home-copy .lede{max-width:40ch}
.route-board{position:relative; margin-top:2px}
...
/* ---------------- footer ---------------- */
.site-foot{border-top:1.5px solid var(--ink); background:var(--paper-2); padding:30px 0 44px}
.mark-f{font-family:var(--display); font-weight:800; font-size:15.5px; letter-spacing:-0.01em}
.foot-nav{display:flex; flex-wrap:wrap; gap:6px 22px; margin-top:10px; font-family:var(--display); font-size:14px; font-weight:700}
.foot-nav a{color:var(--cobalt); text-decoration:none}
.foot-nav a:hover{text-decoration:underline; color:var(--tang-ink)}
.fine{margin-top:12px; font-size:14px; color:var(--ink-2); max-width:78ch}
...
@media (max-width:620px){
  ...
  .fine{font-size:12.5px; line-height:1.42; margin-top:9px}
  .foot-nav{font-size:12.5px; gap:4px 14px}
