...
export function loadSiteConfig(root) {
  ...
  exactKeys(config, ['askFormUrl', 'offerFormUrl', 'addYourselfFormUrl', 'caseStudyProposalUrl', 'interviewUrl'], 'data/site.json', errors);
  for (const key of ['askFormUrl', 'offerFormUrl', 'addYourselfFormUrl', 'caseStudyProposalUrl', 'interviewUrl']) validateHttpsUrl(config?.[key], `data/site.json ${key}`, errors);
