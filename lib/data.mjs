...
export function loadSiteConfig(root) {
  ...
  exactKeys(config, ['askFormUrl', 'offerFormUrl', 'addYourselfFormUrl', 'assessFormUrl'], 'data/site.json', errors);
  for (const key of ['askFormUrl', 'offerFormUrl', 'addYourselfFormUrl', 'assessFormUrl']) validateHttpsUrl(config?.[key], `data/site.json ${key}`, errors);
