...
function unsafeText(value) {
  return typeof value !== 'string' || hiddenOrControl.test(value) || markup.test(value) || unfinishedToken.test(value);
}
...
export function validateHttpsUrl(value, label, errors) {
  if (typeof value === 'string' && value.length > 2048) {
    fail(errors, `${label} must not exceed 2048 characters`);
    return;
  }
  if (typeof value !== 'string' || hiddenOrControl.test(value) || /[\s"'<>]/u.test(value) || /%0[0-9a-f]|%1[0-9a-f]/iu.test(value)) {
...
function findJpegDecoder() {
  if (jpegDecoder) return jpegDecoder;
  const candidates = [
    process.env.DJPEG_BIN,
  ...
  for (const candidate of candidates) {
    ...
    if (!result.error && result.status === 0) {
      jpegDecoder = candidate;
      return jpegDecoder;
    }
  }
  throw new Error('No strict JPEG decoder found. Install djpeg from libjpeg-turbo or set DJPEG_BIN.');
}

function validateJpegDecode(location, label, errors) {
  let decoder;
  try {
    decoder = findJpegDecoder();
  } catch (error) {
    fail(errors, error.message);
    return;
  }
  const result = spawnSync(decoder, [location], {
    stdio: ['ignore', 'ignore', 'pipe'],
    encoding: 'utf8',
    maxBuffer: 64 * 1024,
    timeout: 10_000,
  });
  if (result.error || result.status !== 0) fail(errors, `${label} must pass strict JPEG decoding`);
}
...
export function validateJpeg(location, expectedWidth, expectedHeight, errors, label = location) {
  ...
  while (offset < bytes.length) {
    if (bytes[offset] !== 0xff) {
      fail(errors, `${label} has data outside a JPEG scan`);
      return;
    }
    ...
    if (marker === 0xd9) {
      sawEnd = true;
      if (offset !== bytes.length) fail(errors, `${label} must not contain data after the JPEG end marker`);
      break;
    }
    if (marker === 0x00 || marker === 0xd8 || marker === 0x01 || (marker >= 0xd0 && marker <= 0xd7)) {
      fail(errors, `${label} has an invalid JPEG marker`);
      return;
    }
    ...
    if (marker >= 0xe0 && marker <= 0xef) {
      const isJfif = marker === 0xe0 && !sawJfif && !sawScan && payload.subarray(0, 5).toString('ascii') === 'JFIF\0';
    ...
    if (marker === 0xda) {
      ...
      while (offset < bytes.length) {
        ...
        if (scanMarker === 0x00 || (scanMarker >= 0xd0 && scanMarker <= 0xd7)) {
          if (scanMarker === 0x00) scanDataBytes += 1;
          offset += 1;
          continue;
        }
        offset = markerStart;
      ...
      if (!foundMarker) {
        fail(errors, `${label} has a truncated JPEG scan`);
        return;
      }
      if (scanDataBytes === 0) fail(errors, `${label} has no JPEG scan data`);
    }
  }
  if (!sawJfif) fail(errors, `${label} must use a stripped JFIF JPEG`);
  if (!sawScan) fail(errors, `${label} must contain JPEG image data`);
  if (!sawEnd) fail(errors, `${label} must contain a complete JPEG end marker`);
  if (width !== expectedWidth || height !== expectedHeight) fail(errors, `${label} must be exactly ${expectedWidth}x${expectedHeight}`);
  if (errors.length === initialErrorCount) validateJpegDecode(location, label, errors);
}
