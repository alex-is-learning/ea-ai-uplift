...
export function checkPublicFiles(projectRoot = root) {
  if (fs.existsSync(path.join(projectRoot, '.git'))) {
    const tracked = spawnSync('git', ['-C', projectRoot, 'ls-files', '-z', '--', 'dist'], { encoding: 'buffer' });
    if (tracked.error || tracked.status !== 0) throw new Error('cannot inspect tracked generated output');
    const paths = tracked.stdout.toString('utf8').split('\0').filter(Boolean);
    if (paths.length) throw new Error(`generated output is tracked: ${paths.join(', ')}`);
  }
