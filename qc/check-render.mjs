...
async function renderFile(browser, pagePath, viewport) {
  ...
  try {
    ...
    const state = await cdp.send('Runtime.evaluate', {
      expression: '({width:document.documentElement.scrollWidth,viewport:window.innerWidth,ready:document.readyState,text:document.body.innerText.length,invalidImages:[...document.images].filter((image)=>!image.complete||image.naturalWidth===0).map((image)=>image.src)})',
      returnByValue: true,
    });
