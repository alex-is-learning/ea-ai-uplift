...
async function renderFile(browser, pagePath, viewport) {
  ...
  try {
    ...
    if (!target?.webSocketDebuggerUrl) throw new Error('Chromium supplied no page target');
    ...
    const state = await cdp.send('Runtime.evaluate', {
      expression: '({width:document.documentElement.scrollWidth,viewport:window.innerWidth,ready:document.readyState,text:document.body.innerText.length})',
      returnByValue: true,
    });
